...
const fs = require('fs');
const decodeResource = require(path.resolve('lib','decode-resource'));
const decodeTexture = require(path.resolve('lib','decode-texture'));
const getUrl = require(path.resolve('lib','get-url'));

/**************************** config ****************************/
const PLANET = 'earth';
const URL_PREFIX = `https://kh.google.com/rt/${PLANET}/`;
const DL_DIR = 'downloaded_files';
const [DUMP_JSON_DIR, DUMP_RAW_DIR] = [path.join(DL_DIR, 'json'), path.join(DL_DIR, 'raw')];

const {OCTANT, MAX_LEVEL, DUMP_JSON, DUMP_RAW} = require(path.resolve('lib','parse-command-line'))(process.argv.slice(2));
/****************************************************************/

if (DUMP_JSON || DUMP_RAW) {
	DUMP_JSON && shell.mkdir('-p', DUMP_JSON_DIR);
	DUMP_RAW && shell.mkdir('-p', DUMP_RAW_DIR);
...
async function run() {
	...
	const DIR = path.join(DL_DIR, 'obj', `${OCTANT}-${MAX_LEVEL}-${rootEpoch}`);
	const dir = DIR;
	...
	async function extract(nodePath, prev, exclude) {
		...
		for (let [k, v] of Object.entries(node.meshes)) {
			...
			fs.appendFileSync(path.join(dir,`${texName}.${ext}`), buf);
